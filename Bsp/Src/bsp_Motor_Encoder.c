...
int32_t angle_limiting_int32(int32_t Angl_Err, int32_t lap_encoder);
...
Encoder_t *Encoder_Init(Encoder_Type_e Encoder_Type , uint8_t ch){
    ...
    switch (Encoder_Type) {
       ...
       case M3508_EngineeringPitch:{
           Encoder->lap_encoder = 8192;//编码器单圈码盘值
           Encoder->gear_Ratio = 53;   //减速比
       } break ;
 ...
 */
void CAN_DATA_Encoder_Deal(int32_t position, int16_t speed, uint8_t Encoder_Num) {
    //返回指针
    ...
    //多圈码盘值，不做360度后归零处理
    volatile int erro=angle_limiting_int32(Encoder->position - Encoder->last_position,Encoder->lap_encoder); //差值累加
...
void Briter_Encoder_Code_Set(int16_t CAN_STD_ID, Briter_Encoder_Code_e code,uint32_t data);
...
void Briter_Encoder_Code_Set(int16_t CAN_STD_ID, Briter_Encoder_Code_e code,uint32_t data){
    switch (code) {
        ...
        case Set_Auto_Return_Time:{
           ...
           temp[4] = (uint8_t)((int32_t)data>>8);//高位
        } break;
        case Set_ZeroPoint:{
           temp[0] = 0x04;
           temp[1] = 0x01;
